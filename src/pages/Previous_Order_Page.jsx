  ...
  const getUserOrders = async () => {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:1921/order/user/${userId}`);
  ...
  useEffect(() => {
    ...
    <div key={item._id} className="flex items-center space-x-4 p-4 bg-[#FEF7D7]/30 rounded-lg">
      ...
      <div className="flex-1">
        <h4 className="font-semibold text-[#9C0B13]">{item.product?.name || 'Product Name'}</h4>
      ...
      <div className="text-right">
        <p className="font-bold text-[#9C0B13]">₹{item.product?.price?.toLocaleString() || 0}</p>
        {item.product?.discountedPrice && (
          <p className="text-gray-500 line-through text-sm">₹{item.product.discountedPrice.toLocaleString()}</p>
        )}
      </div>
    </div>
  ))}
    ...
    <div className="space-y-2">
      {selectedOrder.cart?.items?.map((item) => (
        <div key={item._id} className="flex justify-between items-center p-2 bg-gray-50 rounded">
          ...
          <span className="font-semibold">₹{(item.product?.price * item.quantity)?.toLocaleString() || 0}</span>
        </div>
      ))}
    </div>
  </div>
